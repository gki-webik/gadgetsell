...
if (isset($_POST["add_product_iphone_post"])) {
    $name = $_POST["name"];
    $price = $_POST["price"];
    $memory = $_POST["memory"];
    $color = $_POST["color"];
    ...
    $suc = $conn->query("INSERT INTO `telefony_apple`(`id`, `name`, `price`, `memory`, `color`, `properties`, `availability`, `description`, `photo_1`, `photo_all`) VALUES (NULL, '$name', '$price', '$memory', '$color', '$json', '$availability', '$description', '$photo', '$photo_all')");
   print_r("<script>alert('Товар успешно добавлен!'); setTimeout(()=> { document.location.href='/admin';}, 0)</script>");
}