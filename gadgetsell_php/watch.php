...
<body>
    ...
    <main>
        <div class="kroshka_box">
            <div class="kroshka">
                <a href="/">Главная</a> / <a href="/catalog">Каталог</a> / <a href="/watch"
                    class="a_kroshka_active">Apple Watch</a>
            </div>
        </div>
        ...
        <div class="main_catalog_container">
                ...
                <section class="div_details__filter none">
                    <div class="div_details__filter__items">
                        ...
                        <!-- <details class="div_details__filter__item">
                            ...
                            <div class="div_details__filter__answer">
                                <div class="div_details__filter__text">
                                    <lable><input type="checkbox" />Apple Watch</lable>
                                </div>
                            </div>
                        </details>
                        <details class="div_details__filter__item">
                            ...
                            <div class="div_details__filter__answer">
                                <div class="div_details__filter__text">
                                    <lable><input type="checkbox" />Apple S8</lable>
                                    <lable><input type="checkbox" />Apple S9 SiP</lable>
                                    <lable><input type="checkbox" />S8 SiP</lable>
                                </div>
                            </div>
                        </details> -->
            ...
            <div class="main_catalog_box">
                ...
                <div class="main_catalog_box__item__cards">
                    ...
                    if (isset ($_GET["sort"])) {
                        if ($_GET["sort"] == "1") {
                            $suc = $conn->query("SELECT * FROM `apple_watch` ORDER BY `price` ASC");
                        } else if ($_GET["sort"] == "2") {
                            $suc = $conn->query("SELECT * FROM `apple_watch` ORDER BY `price` DESC");
                        } else if ($_GET["sort"] == "3") {
                            $suc = $conn->query("SELECT * FROM `apple_watch` ORDER BY `name` ASC");
                        } else if ($_GET["sort"] == "4") {
                            $suc = $conn->query("SELECT * FROM `apple_watch` ORDER BY `name` DESC");
                        }else if ($_GET["sort"] == "5") {
                            $suc = $conn->query("SELECT * FROM `apple_watch` ORDER BY `id` ASC");
                        }else if ($_GET["sort"] == "6") {
                            $suc = $conn->query("SELECT * FROM `apple_watch` ORDER BY `id` DESC");
                        } else {
                            $suc = $conn->query("SELECT * FROM `apple_watch`");
                        }
                    } else {
                        $suc = $conn->query("SELECT * FROM `apple_watch`");
                    }
                    if ($suc->num_rows > 0) {
                        while ($row = $suc->fetch_assoc()) {
                            ...
                            $photo_1 = $row["photo_1"];
                            $result_card = "<div class='item__cards__block ctg_$color'><div class='item__cards__img' onclick=\"window.open('/watch/page?id_page=$id', '_parent')\" style='width: 100%;background-size:cover;height: 225px;text-align: center;background-position: center;background-image: url(https://store77.net$photo_1);'></div><div class='item__cards__text' onclick=\"window.open('/watch/page?id_page=$id', '_parent')\">$name</div><div class='item__cards__price' onclick=\"window.open('/watch/page?id_page=$id', '_parent')\">$price ₽</div><button class='item__cards__btn item__cards__btn_$id'>В корзину</button></div><script>
                            document.querySelector('.item__cards__btn_$id').addEventListener('click', () => { 
                                if(localStorage.getItem('products_basket')){
                                    ...
                                    if(pp_$id === undefined){
                                        arr_b_$id.push({\"id\": \"$id\", \"img\": \"$photo_1\", \"name\": \"$name\", \"price\": \"$price\", \"product\": \"watch\", \"count\": 1});
                                    ...
                                    localStorage.setItem('products_basket', '[{\"id\": \"$id\", \"img\": \"$photo_1\", \"name\": \"$name\", \"price\": \"$price\", \"product\": \"watch\", \"count\": 1}]');
                                } 
                            });
                            </script>";
                            print_r($result_card);
    ...
    <script>
        ...
        let ctgm_Violet = document.querySelector("#ctg_Violet");
        let ctgm_White = document.querySelector("#ctg_White");
        let ctgm_Pink = document.querySelector("#ctg_Pink");
        let ctgm_Yellow = document.querySelector("#ctg_Yellow");
        let ctgm_Black = document.querySelector("#ctg_Black");
        let ctgm_Gray = document.querySelector("#ctg_Gray");
        let ctgm_Silver = document.querySelector("#ctg_Silver");
        ...
        ctg_Silver.addEventListener("change", () => {
            if (!ctg_Silver.checked && !ctg_Gray.checked && !ctg_Black.checked && !ctg_Yellow.checked && !ctg_Pink.checked && !ctg_White.checked && !ctg_Violet.checked) {
                item__cards__block_11.forEach((e) => { e.classList.add("d_flex_2"); });
                item__cards__block_12.forEach((e) => { e.classList.add("d_flex_2"); });
                item__cards__block_13.forEach((e) => { e.classList.add("d_flex_2"); });
                item__cards__block_14.forEach((e) => { e.classList.add("d_flex_2"); });
        ...
        ctgm_Silver.addEventListener("change", () => {
            if (!ctgm_Silver.checked && !ctgm_Gray.checked && !ctgm_Black.checked && !ctgm_Yellow.checked && !ctgm_Pink.checked && !ctgm_White.checked && !ctgm_Violet.checked) {
